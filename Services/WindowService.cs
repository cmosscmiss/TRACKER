...
public class WindowService
{
    ...
    #region Screenshot (Windows.Graphics.Capture)
    /// <summary>Interop para crear un <c>GraphicsCaptureItem</c> a partir de un HWND (sin selector del sistema).</summary>
    [ComImport]
    ...
    /// <summary>
    /// Captura la ventana ACTIVA ya compuesta (incluye el overlay de Composition y el WebView2) y devuelve un JPEG.
    /// Usa <c>Windows.Graphics.Capture</c>; captura un único frame y cierra la sesión de inmediato. Devuelve
    /// <c>null</c> si no hay ventana activa o si la captura falla (el llamante decide qué hacer).
    /// </summary>
    public async Task<byte[]?> CaptureActiveWindowJpegAsync()
    {
        ...
        Windows.Graphics.DirectX.Direct3D11.IDirect3DDevice? device = null;
        Windows.Graphics.Capture.Direct3D11CaptureFramePool? framePool = null;
        ...
        try
        {
            ...
            IGraphicsCaptureItemInterop interop = WinRT.ActivationFactory
                .Get("Windows.Graphics.Capture.GraphicsCaptureItem")
                .AsInterface<IGraphicsCaptureItemInterop>();
            Guid itemIid = new("79C3F95B-31F7-4EC2-A464-632EF5D30760");   // IGraphicsCaptureItem
            IntPtr itemPtr = interop.CreateForWindow(hWnd, ref itemIid);
            Windows.Graphics.Capture.GraphicsCaptureItem item = Windows.Graphics.Capture.GraphicsCaptureItem.FromAbi(itemPtr);
            ...
            framePool = Windows.Graphics.Capture.Direct3D11CaptureFramePool.CreateFreeThreaded(
                device, Windows.Graphics.DirectX.DirectXPixelFormat.B8G8R8A8UIntNormalized, 1, size);
            session = framePool.CreateCaptureSession(item);

            // Suprimir el borde de captura si la API está disponible (Win11+); vía reflexión para no romper el
            // compilado en SDKs donde la propiedad no exista.
            try { typeof(Windows.Graphics.Capture.GraphicsCaptureSession).GetProperty("IsBorderRequired")?.SetValue(session, false); }
            catch { /* no disponible: se captura igual */ }

            var tcs = new TaskCompletionSource<Windows.Graphics.Capture.Direct3D11CaptureFrame?>();
            Windows.Foundation.TypedEventHandler<Windows.Graphics.Capture.Direct3D11CaptureFramePool, object> handler =
                (pool, _) => tcs.TrySetResult(pool.TryGetNextFrame());
            ...
            using Windows.Graphics.Capture.Direct3D11CaptureFrame? frame = await tcs.Task;
            if (frame is null)
                return null;

            using Windows.Graphics.Imaging.SoftwareBitmap raw = await Windows.Graphics.Imaging.SoftwareBitmap.CreateCopyFromSurfaceAsync(frame.Surface);
            using Windows.Graphics.Imaging.SoftwareBitmap jpegReady = Windows.Graphics.Imaging.SoftwareBitmap.Convert(
                raw, Windows.Graphics.Imaging.BitmapPixelFormat.Bgra8, Windows.Graphics.Imaging.BitmapAlphaMode.Ignore);

            using var stream = new Windows.Storage.Streams.InMemoryRandomAccessStream();
            Windows.Graphics.Imaging.BitmapEncoder encoder = await Windows.Graphics.Imaging.BitmapEncoder.CreateAsync(
                Windows.Graphics.Imaging.BitmapEncoder.JpegEncoderId, stream);
            encoder.SetSoftwareBitmap(jpegReady);
    ...
    /// <summary>Crea un <c>IDirect3DDevice</c> WinRT (hardware, con fallback a WARP) para el frame pool de captura.</summary>
    private static Windows.Graphics.DirectX.Direct3D11.IDirect3DDevice CreateDirect3DDevice()
    {
        const int driverHardware = 1;
        const int driverWarp = 5;
        const uint bgraSupport = 0x20;
        const uint sdkVersion = 7;

        int hr = D3D11CreateDevice(IntPtr.Zero, driverHardware, IntPtr.Zero, bgraSupport, IntPtr.Zero, 0, sdkVersion, out IntPtr devicePtr, out _, out IntPtr ctxPtr);
        if (hr != 0)
            hr = D3D11CreateDevice(IntPtr.Zero, driverWarp, IntPtr.Zero, bgraSupport, IntPtr.Zero, 0, sdkVersion, out devicePtr, out _, out ctxPtr);
        if (hr != 0)
        ...
        Marshal.QueryInterface(devicePtr, ref iidDxgiDevice, out IntPtr dxgiPtr);
        try
        {
            int hr2 = CreateDirect3D11DeviceFromDXGIDevice(dxgiPtr, out IntPtr graphicsPtr);
            if (hr2 != 0)
                throw new InvalidOperationException($"CreateDirect3D11DeviceFromDXGIDevice failed: 0x{hr2:X8}");
            try
            {
                return WinRT.MarshalInterface<Windows.Graphics.DirectX.Direct3D11.IDirect3DDevice>.FromAbi(graphicsPtr);
            }
